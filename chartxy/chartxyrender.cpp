...
void ChartXYRender::paintGrid(QPainter &painter)
  ...
  rect.adjust(m_setting->border().left(),
              ...
              -int(m_setting->border().right()),
              -int(m_setting->border().bottom())
              );
...
void ChartXYRender::paintGridLineX(QPainter &painter,QRect &rect)
  ...
  // Adjust axis border
  rect_axis.adjust(m_setting->grid().border(),0,-int(m_setting->grid().border()),0);
  point_major_top=rect_axis.topLeft();
  point_major_bottom=rect_axis.bottomLeft();


  // Draw major tick
  for(i=0;i<m_setting->grid().horizontalTick().tickMajor();i++)
    {
    QPointF point_delta=QPointF((((qreal)rect_axis.width()-1)*(i))/(m_setting->grid().horizontalTick().tickMajor()-1),0);
...
void ChartXYRender::paintGridLineY(QPainter &painter,QRect &rect)
  ...
  // Adjust axis border
  rect_axis.adjust(0,m_setting->grid().border(),0,-int(m_setting->grid().border()));
...
void ChartXYRender::paintAxisX(QPainter &painter,QRect &rect)
  ...
  for(i=0;i<m_setting->grid().horizontalTick().tickMajor();i++)
    ...
    point_major+=QPointF((((qreal)rect_axis.width())*(i))/(m_setting->grid().horizontalTick().tickMajor()-1),0);

    // Draw minor tick
    if(i<m_setting->grid().horizontalTick().tickMajor()-1)
      {
      for(j=0;j<m_setting->grid().horizontalTick().tickMinor();j++)
        {
        QPointF point_minor=point_major;

        point_minor+=QPointF(((((qreal)rect_axis.width())/(m_setting->grid().horizontalTick().tickMajor()-1)*j)/(m_setting->grid().horizontalTick().tickMinor()-1)),0);
...
void ChartXYRender::paintAxisXTickvalue(QPainter &painter,QRect &rect)
  ...
  // Adjust axis border
  rect_axis.adjust(m_setting->grid().border(),0,-int(m_setting->grid().border()),0);
  half_text_length=((((qreal)rect_axis.width()))/(m_setting->grid().horizontalTick().tickMajor()-1))/3.0;

  for(i=0;i<m_setting->grid().horizontalTick().tickMajor();i++)
    {
    QString text;
    QPointF point=rect_axis.bottomLeft();
    point+=QPointF((((qreal)rect_axis.width())*(i))/(m_setting->grid().horizontalTick().tickMajor()-1)-half_text_length,12);
    QRectF text_rect(point,point+QPoint(half_text_length*2,pixel_size));

    text=tickValueToString(tickValue(m_setting->scale().currentLimit().limitX(),m_setting->grid().horizontalTick(),i));
